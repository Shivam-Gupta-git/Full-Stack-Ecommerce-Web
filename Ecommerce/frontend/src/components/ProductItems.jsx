...
function ProductItems({items}) {
  ...
  return (
    ...
    <div className=" w-40 sm:w-55 md:w-60  lg:w-62 h-auto bg-white rounded-md shadow-sm hover:shadow-md transition duration-300 ">
      <Link to={`/product/${items._id}`} className="block">
  
        {/* Image Section */}
        <div className="w-full h-40 sm:h-56 md:h-56 lg:h-60 overflow-hidden rounded-t-md">
          <img
            src={items.image[0]}
            alt={items.name}
