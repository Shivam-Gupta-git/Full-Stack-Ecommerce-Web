...
function AdminLogin({setToken}) {
  ...
  try {
    ...
    const response = await axios.post(backentUrl + '/api/user/loginAdmin',{email, password})
  ...
  <div className="bg-white shadow-lg rounded-2xl p-8 w-full max-w-md">
    ...
    <form onSubmit={handelAdminDetails} method="POST" className="space-y-5">

      <div>
        <label  className="block text-sm font-medium text-gray-700">Email address</label>
        <input type="email"  required placeholder='Enter your email' onChange={(event)=> setEmail(event.target.value)} value={email}
      ...
      <div>
        ...
        <input type="password"  required placeholder='Enter your password' onChange={(event) => setPassword(event.target.value)} value={password}
